...
variable "resource_group_name" {
  description = "rkm-<계정 이름>-kafka-rg. 전달받은 계정이 student07 이면 rkm-student07-kafka-rg."
  type        = string

  validation {
    condition     = can(regex("^rkm-[a-z0-9]+-kafka-rg$", var.resource_group_name))
    error_message = "rkm-<계정 이름>-kafka-rg 형식이어야 한다. 다른 수강생의 그룹을 건드리지 않게 막는다."
  }
}
